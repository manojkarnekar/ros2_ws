...
class Move_base_goal(object):
    ...
    def movebase_client(self, p_x ,p_y, o_z, o_w):
        ...
        if not self.distance_tolerance > 0.0:
            ...
            finished_within_time = self.client.wait_for_result(rospy.Duration(100.0))
            ...
            while(distance > self.distance_tolerance):
                now = rospy.Time.now()
                self.listener.waitForTransform(self.odom_frame_id, self.base_frame_id, now, rospy.Duration(2.0))
