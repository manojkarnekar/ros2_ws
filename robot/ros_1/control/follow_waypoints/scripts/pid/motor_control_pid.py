...
import numpy as np
import os
# os.system("rostopic pub /steer_speed std_msgs/UInt8 'data: 75'")
...
class TwistToMotor():
    ...
    def spinOnce(self):
        ...
        # # print(self.steer, self.actual_steer)

        self.pub_steer_angle.publish(self.steer)

        # if abs(self.steer) > 1.0:
        #     print("self.steer--->", self.steer)
        #     # self.drive = self.K_drive * (math.hypot(self.bl*self.steer, 0.049))
        #     self.drive = self.K_drive * 0.09
    ...
    def twistCallback(self,msg):
        ...
        if self.Vx or self.Wz == 0:
            self.pub_steer_angle.publish(0)
            self.pub_drive_motor.publish(0)
