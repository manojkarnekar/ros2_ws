    ...
    while (1) {
        if (zed.grab() == sl::ERROR_CODE::SUCCESS) {
            ...
            for (size_t j = 0; j < res.size(); j++) {
                ...
                cv::line(left_cv_rgb, cv::Point(r.x + r.width/2 , r.y + r.height/2) , cv::Point(left_cv_rgb.cols/2 , r.y + r.height/2) ,cv::Scalar(255, 255, 0), 3, cv::LINE_4);


                // Left most point of bounding box
                draw_mid_points(left_cv_rgb, r, point_cloud, baseline, arr, ch = 1);

                d_left = arr[3];


                // right most point of bounding box
                draw_mid_points(left_cv_rgb, r, point_cloud, baseline, arr, ch = 2);
                ...
                draw_points_cluster(left_cv_rgb, r, point_cloud, cluster_arr, ch = 1);

                float left_min_x = 0, left_min_y = 0, left_min_z = 0;

                left_min_x = cluster_arr[0];
                left_min_y = cluster_arr[1];
                left_min_z = cluster_arr[2];


                // for points cluster to the right of right point of bounding box
                draw_points_cluster(left_cv_rgb, r, point_cloud, cluster_arr, ch = 2);

                float right_min_x = 0, right_min_y = 0, right_min_z = 0;

                right_min_x = cluster_arr[0];
                right_min_y = cluster_arr[1];
                right_min_z = cluster_arr[2];


                // for points cluster around left point of bounding box
                draw_points_cluster(left_cv_rgb, r, point_cloud, cluster_arr, ch = 3);

                float left_bb_min_x = 0, left_bb_min_y = 0, left_bb_min_z = 0;

                left_bb_min_x = cluster_arr[0];
                left_bb_min_y = cluster_arr[1];
                left_bb_min_z = cluster_arr[2];


                // for points cluster around right point of bounding box
                draw_points_cluster(left_cv_rgb, r, point_cloud, cluster_arr, ch = 4);

                float right_bb_min_x = 0, right_bb_min_y = 0, right_bb_min_z = 0;

                right_bb_min_x = cluster_arr[0];
                right_bb_min_y = cluster_arr[1];
                right_bb_min_z = cluster_arr[2];


                // draw depth points along mid line of bounding box
                std::vector<float> depth_points;

                int point_cluster = 20;
                ...
                // depth_points.push_back(distance);

                // cv::circle(left_cv_rgb, cv::Point(point_bb,r.y + r.height/2),5, (255, 0, 255), -1);

                for(float i = r.x; i < r.x + r.width; i=i+r.width/point_cluster)
                {
                    val = 0;
                    point_bb = 0;
                    do
                ...
                // for(float i = r.x; i < r.x + r.width; i=i+r.width/point_cluster)
                // {
                //     point_cloud.getValue(i , r.y + r.height/2 , &point_cloud_value);
                //     distance = sqrt(point_cloud_value.x*point_cloud_value.x + point_cloud_value.y*point_cloud_value.y + point_cloud_value.z*point_cloud_value.z);

                //     depth_points.push_back(distance);

                //     cv::circle(left_cv_rgb, cv::Point(i, r.y + r.height/2),5, (255, 0, 255), -1);
                // }

                std::cout << depth_points.size() << std::endl;

                
                
                // size measure of left half of bed
                float bed_left_half;
                bed_left_half = sqrt(((bed_mid_pcx - left_bb_min_x) * (bed_mid_pcx - left_bb_min_x)) + ((bed_mid_pcy - left_bb_min_y) * (bed_mid_pcy - left_bb_min_y)) + ((bed_mid_pcz - left_bb_min_z) * (bed_mid_pcz - left_bb_min_z)));

                // size measure of right half of bed
                float bed_right_half;
                bed_right_half = sqrt(((bed_mid_pcx - right_bb_min_x) * (bed_mid_pcx - right_bb_min_x)) + ((bed_mid_pcy - right_bb_min_y) * (bed_mid_pcy - right_bb_min_y)) + ((bed_mid_pcz - right_bb_min_z) * (bed_mid_pcz - right_bb_min_z)));

                // write points into csv file
                // myfile.open (argv[3]);
                // myfile << left_bb_min_x/1000 << "," << left_bb_min_z/1000 << "," << min_x/1000 << "," << min_z/1000 << "," << right_bb_min_x/1000 << "," << right_bb_min_z/1000 << "," << bed_left_half/1000 << "," << object_distance/1000 << "," << bed_right_half/1000 << "," << bb_mid_point_y/1000 << "\n";
                // myfile.close();
                ...
                for(int i =0;i < depth_points.size(); i++)
                {
                    ...
                    if(i == depth_points.size() - 1)
                    {
                        // myfile << depth_points.at(i)/1000 << "\n";
                        myfile << depth_points.at(i)/1000 ;
                    }
                ...
                // myfile << depth_points.at(depth_points.back())/1000 << "\n";
                myfile << "\n";
                myfile.close();
