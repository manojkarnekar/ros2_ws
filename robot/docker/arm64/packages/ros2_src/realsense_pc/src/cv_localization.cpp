...
void QR_code_detection::display(cv::Mat &im, vector<decodedObject>&decodedObjects, rs2::depth_frame depth)
{
  for(int i = 0; i < decodedObjects.size(); i++)
  {
    ...
    x[0] = pixel_to_point(depth, {pt[0].x, pt[0].y}, 70)[0];
    y[0] = pixel_to_point(depth, {pt[0].x, pt[0].y}, 70)[1];
    z[0] = pixel_to_point(depth, {pt[0].x, pt[0].y}, 70)[2];
    point[0] = {x[0], y[0], z[0]};
    ...
    for(int i = 1; i < num; i++)
    {
      ...
      // {
      //   try{
        x[i] = pixel_to_point(depth, {pt[i].x, pt[i].y}, 20)[0];
        y[i] = pixel_to_point(depth, {pt[i].x, pt[i].y}, 20)[1];
        z[i] = pixel_to_point(depth, {pt[i].x, pt[i].y}, 20)[2];
        // }
        // catch (const std::exception& e)
        // {
        //   std::cerr << e.what() << std::endl;
  ...
  //   qx = stof(pose[1][3]);
  //   qy = stof(pose[1][4]);
  //   qz = stof(pose[1][5]);               // waypoints orientation in quaternion
  //   qw = stof(pose[1][6]);
  ...
  //   //   t_x = (z[0] + t_x1)*cos(alpha_0);
  //   //   t_y = (z[0] + t_x1)*sin(alpha_0); 
  //   // }

  //   // if(alpha >= 0)
  //   // {

  //   // vector<float> alpha_vector{};
  //   // float alp;
  ...
  //   // // else if(alpha > 2.0)   //alpha_vector.size() < 30 &&
  //   // // {
  //   // else
  //   // {
  //   //   alpha_vector.push_back(alpha);
  //   //   // alpha_vector.push_back(1.0);
  //   //   cout << " alpha_vector.size() = " << alpha_vector.size() << endl;
  //   // }
  //   // // }  

  //   // // else if(alpha < -2.0 && alpha != 0)    //alpha_vector.size() < 30 &&
  //   // // {
  //   // //   alpha_vector.push_back(-alpha);
  //   // //   cout << "negative alpha" << endl;
  //   // //   cout << " alpha_vector.size() = " << alpha_vector.size() << endl;
  //   // // } 

  //   // cout << " alpha_vector.size() = " << alpha_vector.size() << endl;

  //   // if( alpha > 2.0)   //alpha_vector[29] != 0 &&   //alpha_vector.size() > 30 && 
  //   // {
  //   //     vector<float> vec_ = minmax_idx(alpha_vector);
  //   //     int idx_ = vec_.at(0);

  //   //     alp = alpha_vector[idx_];
  //   //     // cout << "alpha_vector[30] = " << alpha_vector[30] << endl;
  //   //     cout << " alpha_vector.size() = " << alpha_vector.size() << endl;
  //   //     // alpha_vector.clear();
  //   // }
  //   // else if(alpha_vector.size() > 30 && alpha < -2.0)    //alpha_vector[29] != 0 &&
  //   // {
  //   //     vector<float> vec_ = minmax_idx(alpha_vector);
  //   //     int idx_ = vec_.at(1);
  //   //     alp = -alpha_vector[idx_];
  ...
  //   t_x = (z[0] + t_x1)*cos(-alpha*3.14/180.0);
  //   t_y = (z[0] + t_x1)*sin(-alpha*3.14/180.0); 
  //   // }   
  //   double alpha_1 = (180.0 - alpha)*3.14/180.0;
