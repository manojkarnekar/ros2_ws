#include "teach_and_repeat/core0_repeat.h"
...
void core0R::sendGoal(void) 
{
    ...
    if (send_pose_idx == 0)
    {
      startNav2sendGoal(stod(pose[1][0]),stod(pose[1][1]), stod(pose[1][2]), stod(pose[1][3]), stod(pose[1][4]), stod(pose[1][5]), stod(pose[1][6]));
    }
    ...
    if (send_pose_idx == 1)
    {
      ...
      for (int i = 1; i <= stod(goal_pose_idx[send_pose_idx][0]); i+=1)
      {
        goal_msg.poses.push_back(sendNav2Goal(stod(pose[i][0]),stod(pose[i][1]), stod(pose[i][2]), stod(pose[i][3]), stod(pose[i][4]), stod(pose[i][5]), stod(pose[i][6])));
      }
    ...
    if (send_pose_idx > 1)
    {
      ...
      int start_pose = stod(goal_pose_idx[prev_pose][0]);
      ...
      for (int i = start_pose; i <= stod(goal_pose_idx[send_pose_idx][0]); i+=1)
      {
        goal_msg.poses.push_back(sendNav2Goal(stod(pose[i][0]),stod(pose[i][1]), stod(pose[i][2]), stod(pose[i][3]), stod(pose[i][4]), stod(pose[i][5]), stod(pose[i][6])));
      }
      auto send_goal_options = rclcpp_action::Client<NavigateThroughPoses>::SendGoalOptions();
...
void core0R::core0_repeat()
{
    ...
    {
        acummulated_poses_.push_back(sendNav2Goal(stod(pose[i][0]),stod(pose[i][1]), stod(pose[i][2]), stod(pose[i][3]), stod(pose[i][4]), stod(pose[i][5]), stod(pose[i][6])));
    }
