        ...
        if (zed.grab() == sl::ERROR_CODE::SUCCESS) {
            ...
            for (int c = 0; c < NUM_CLASSES; c++) {
                for (size_t i = 0; i < indices[c].size(); ++i) {
                    ...
                    bed_mid_pcx = point_cloud_value.x;
                    bed_mid_pcy = point_cloud_value.y;
                    bed_mid_pcz = point_cloud_value.z;
                    ...
                    std::cout << "Mid Point of bed : " << (rect.x + rect.width/2) << std::endl;
                    std::cout << "Frame mid point : " << (frame.cols/2) << std::endl;

                    cv::line(frame, cv::Point(rect.x + rect.width/2 , rect.y + rect.height/2) , cv::Point(frame.cols/2 , rect.y + rect.height/2) ,cv::Scalar(255, 255, 0), 3, cv::LINE_4);

                    
                    // mid-point of image frame with height same as bounding box mid point
                    // val = 0;
                    // do
                    // {
                    //     point_cloud.getValue(frame.cols/2 + val , rect.y + rect.height/2, &point_cloud_value);
                    //     distance = sqrt(point_cloud_value.x*point_cloud_value.x + point_cloud_value.y*point_cloud_value.y + point_cloud_value.z*point_cloud_value.z);
                    //     val = val + 1;
                    // }while(isnan(distance)==true);

                    // float frame_point_y = point_cloud_value.x/1000;

                    // dist = std::to_string(frame_point_y/1000);
                    // cv::putText(frame, dist, cv::Point(frame.cols/2 - 20, rect.y + rect.height/2 + 50),cv::FONT_HERSHEY_COMPLEX_SMALL, 3, cv::Scalar(255, 255, 0), 3);

                    // mid-point of bounding box
                    val = 0;
                    do
                    {
                        point_cloud.getValue(rect.x + rect.width/2 + val, rect.y + rect.height/2, &point_cloud_value);
                        distance = sqrt(point_cloud_value.x*point_cloud_value.x + point_cloud_value.y*point_cloud_value.y + point_cloud_value.z*point_cloud_value.z);
                        val = val + 1;
                    }while(isnan(distance)==true);

                    float bb_mid_point_y = point_cloud_value.x;
                    

                    dist = std::to_string(bb_mid_point_y/1000);
                    cv::putText(frame, dist, cv::Point(frame.cols/2, rect.y + rect.height/2 + 50),cv::FONT_HERSHEY_COMPLEX_SMALL, 2.5, cv::Scalar(255, 255, 0), 3);
                    ...
                    */
                    ...
                    // myfile.open ("coordinates.csv");
                    // myfile << left_min_x/1000 << "," << left_min_z/1000 << "," << min_x/1000 << "," << min_z/1000 << "," << right_min_x/1000 << "," << right_min_z/1000 << "," << bed_left_half << "," << bed_right_half << "\n";
                    // myfile.close();
                    ...
                    for(float i = rect.x - (1 * (0.05 * rect.width)); i < rect.x + (1 * (0.1 * rect.width)); i=i+25)
                    {
                    ...
                    for(float i = rect.x + rect.width - (1 * (0.1 * rect.width)); i < rect.x + rect.width + (1 * (0.05 * rect.width)); i=i+25)
                    {
                        for(float j = (rect.y + rect.height/2) - (0.4* rect.height); j < (rect.y + rect.height/2) + (0.4* rect.height); j=j+25)
                        {
                            if(i < frame.cols)
                    ...
                    myfile.open ("coordinates.csv");
                    myfile << left_min_x/1000 << "," << left_min_z/1000 << "," << min_x/1000 << "," << min_z/1000 << "," << right_min_x/1000 << "," << right_min_z/1000 << "," << bed_left_half/1000 << "," << object_distance/1000 << "," << bed_right_half/1000 << "," << bb_mid_point_y/1000 << "\n";
                    myfile.close();
